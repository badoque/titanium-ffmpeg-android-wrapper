...
#define TAG "com.badoque.ffmpeg.android"

using namespace v8;

static Persistent<Object> bindingCache;

static void FfmpegAndroidWrapper_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "FfmpegAndroidWrapper.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::FfmpegAndroidWrapperBindings::lookupGeneratedInit(
...
static void FfmpegAndroidWrapper_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, FfmpegAndroidWrapper_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void FfmpegAndroidWrapper_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::FfmpegAndroidWrapperBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry FfmpegAndroidWrapperBinding = {
	"com.badoque.ffmpeg.android",
	FfmpegAndroidWrapper_init,
	FfmpegAndroidWrapper_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_badoque_ffmpeg_android_FfmpegAndroidWrapperBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.badoque.ffmpeg.android", &FfmpegAndroidWrapperBinding);
	titanium::KrollBindings::addExternalLookup(&(::FfmpegAndroidWrapperBindings::lookupGeneratedInit));
}
